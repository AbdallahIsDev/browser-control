...
export class AIAgent {
  ...
  constructor(options: AIAgentOptions = {}) {
    ...
    this.model = options.model ?? process.env.AI_AGENT_MODEL ?? process.env.OPENROUTER_MODEL ?? "openai/gpt-4.1-mini";
    ...
    this.costPerToken = Number(process.env.AI_AGENT_COST_PER_TOKEN ?? "0.0001");
  }
  ...
  async executeGoal(goal: string, pageOrSessionId: Page | string, maxSteps = 10): Promise<GoalExecutionResult> {
    ...
    for (let step = 1; step <= effectiveMaxSteps; step += 1) {
      ...
      console.log(`[AI_AGENT] Step ${step}: ${decision.action} (${decision.rationale})`);
  ...
  private async decide(goal: string, description: AgentPageDescription): Promise<AgentDecision> {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt += 1) {
      ...
      try {
        ...
        console.log(`[AI_AGENT] JSON parse failed (attempt ${attempt}/${maxRetries}): ${lastError.message}`);
  ...
  private async executeDecision(page: Page, decision: AgentDecision): Promise<void> {
    ...
    if (this.guardrails.dryRun) {
      ...
      console.log(`[DRY_RUN] Would execute: ${decision.action} on ${selector}`);
      return;
    }
    ...
    if (this.guardrails.deniedSelectors && decision.selector) {
      for (const pattern of this.guardrails.deniedSelectors) {
        if (selectorMatchesPattern(decision.selector, pattern)) {
          throw new GuardrailError(
            `[GUARDRAIL] Selector "${decision.action}" on "${decision.selector}" matches denied pattern "${pattern}"`,
            "deniedSelectors",
          );
        }
      }
    ...
    if (this.guardrails.requireConfirmation && isDestructiveAction(decision)) {
      console.log(`[GUARDRAIL] Confirmation required for: ${decision.action} on ${decision.selector}`);
      return;
    }
