...
const CONFIG_DEFINITIONS: ConfigDefinition[] = [
  ...
  { key: "modelProvider", category: "ai", envVars: ["BROWSER_CONTROL_MODEL_PROVIDER"], description: "Preferred model provider used by the Browser Control model router. Choose `openrouter`, `ollama`, or `openai-compatible` based on where model requests should be sent.", defaultValue: () => "openrouter", parse: requiredString, validate: ensureAllowed(["openrouter", "ollama", "openai-compatible"]) },
  { key: "modelEndpoint", category: "ai", envVars: ["BROWSER_CONTROL_MODEL_ENDPOINT"], description: "Custom OpenAI-compatible base URL for the selected model provider. Use this for local gateways, private deployments, or providers that are not covered by built-in defaults.", defaultValue: () => undefined, parse: optionalString, validate: ensureUrl },
  { key: "modelApiKey", category: "ai", envVars: ["BROWSER_CONTROL_MODEL_API_KEY"], description: "API key for the selected model provider in the model router. This value is sensitive and is redacted from config output.", defaultValue: () => undefined, parse: optionalString, sensitive: true },
  { key: "modelName", category: "ai", envVars: ["BROWSER_CONTROL_MODEL_NAME"], description: "Model identifier used by the selected model provider. Use provider-specific names such as an OpenRouter model slug or a local Ollama model name.", defaultValue: () => undefined, parse: optionalString },
  { key: "openrouterModel", category: "ai", envVars: ["OPENROUTER_MODEL", "AI_AGENT_MODEL"], description: "OpenRouter model slug used by legacy AI-agent features. This remains for compatibility with older agent configuration paths.", defaultValue: () => "openai/gpt-4.1-mini", parse: requiredString },
  { key: "openrouterBaseUrl", category: "ai", envVars: ["OPENROUTER_BASE_URL"], description: "Base URL for OpenRouter-compatible API requests. Override this only for a compatible proxy or self-hosted gateway.", defaultValue: () => "https://openrouter.ai/api/v1", parse: requiredString, validate: ensureUrl },
  { key: "openrouterApiKey", category: "ai", envVars: ["OPENROUTER_API_KEY"], description: "API key for OpenRouter-backed AI-agent features. This is sensitive and is redacted from inventories and logs.", defaultValue: () => undefined, parse: optionalString, sensitive: true },
];
