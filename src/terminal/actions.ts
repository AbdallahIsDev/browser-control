 ...
 *   open, exec, type, snapshot, read, interrupt, close, resize
 *
 * Uses:
...
export function validateResize(
	cols: unknown,
	rows: unknown,
): { valid: false; error: string } | ResizeValidationResult {
	if (typeof cols !== "number" || typeof rows !== "number") {
		return { valid: false, error: "cols and rows must be numbers" };
	}
	if (!Number.isFinite(cols) || !Number.isFinite(rows)) {
		return { valid: false, error: "cols and rows must be finite" };
	}
...
export interface TermResizeOptions {
	/** Terminal session ID. */
	sessionId: string;
	/** Terminal columns. */
	cols: number;
...
export class TerminalActions {
	...
	onData(listener: (sessionId: string, data: string) => void): {
		...
		return this.getTerminalRuntime().onData(listener);
	}
	...
	private getTerminalRuntime(): TerminalRuntime {
		return (
			this.context.terminalRuntime ??
			this.context.sessionManager.getTerminalRuntime()
		);
	}
	...
	private async requireDaemonRuntime(
		...
		if (runtime instanceof LocalTerminalRuntime) {
			...
			return this.failureWithDebug(message, new Error(message), {
				action,
	...
	private invalidateBrokerRuntimeOnError(error: unknown): void {
		...
		if (
			/ECONNREFUSED|ECONNRESET|EPIPE|fetch failed|network error|socket hang up/i.test(
				message,
			)
		) {
	...
	private async rejectInvalidTerminalSessionId<T>(
		...
		if (
			!terminalSessionId ||
			TERMINAL_SESSION_ID_PATTERN.test(terminalSessionId)
		) {
			return null;
		}
	...
	> {
		...
		try {
			...
			const daemonGuard = await this.requireDaemonRuntime(
				policyEval,
				sessionId,
				"terminal_open",
			);
			...
			log.info("Terminal session opened", {
				id: result.id,
				shell: result.shell,
			});
	...
	async exec(options: TermExecOptions): Promise<ActionResult<ExecResult>> {
		...
		if (!isPolicyAllowed(policyEval))
			return policyEval as ActionResult<ExecResult>;
		const invalidSession =
			await this.rejectInvalidTerminalSessionId<ExecResult>(
				options.sessionId,
				policyEval,
				"terminal_exec",
			);
	...
	async type(
		...
		const invalidSession = await this.rejectInvalidTerminalSessionId<{
			typed: string;
		}>(options.sessionId, policyEval, "terminal_write");
		if (invalidSession) return invalidSession;
	...
	async read(
		...
		if (!isPolicyAllowed(policyEval))
			return policyEval as ActionResult<{ output: string }>;
		const invalidSession = await this.rejectInvalidTerminalSessionId<{
			output: string;
		}>(options.sessionId, policyEval, "terminal_read");
		if (invalidSession) return invalidSession;
	...
	async snapshot(
		...
		if (!isPolicyAllowed(policyEval))
			return policyEval as ActionResult<TerminalSnapshot | TerminalSnapshot[]>;
		const invalidSession = await this.rejectInvalidTerminalSessionId<
			TerminalSnapshot | TerminalSnapshot[]
		>(options.sessionId, policyEval, "terminal_snapshot");
		if (invalidSession) return invalidSession;
		...
		try {
			...
			if (daemonGuard)
				return daemonGuard as ActionResult<
					TerminalSnapshot | TerminalSnapshot[]
				>;
			...
			return successResult(
				snapResult as TerminalSnapshot | TerminalSnapshot[],
				{
					path: policyEval.path,
					sessionId,
					policyDecision: policyEval.policyDecision,
	...
	async interrupt(
		...
		if (!isPolicyAllowed(policyEval))
			return policyEval as ActionResult<{ interrupted: boolean }>;
		const invalidSession = await this.rejectInvalidTerminalSessionId<{
			interrupted: boolean;
		}>(options.sessionId, policyEval, "terminal_interrupt");
		if (invalidSession) return invalidSession;
		...
		try {
			...
			if (daemonGuard)
				return daemonGuard as ActionResult<{ interrupted: boolean }>;
	...
	async resize(
		...
		if (!isPolicyAllowed(policyEval))
			return policyEval as ActionResult<{ resized: boolean }>;
		const invalidSession = await this.rejectInvalidTerminalSessionId<{
			resized: boolean;
		}>(options.sessionId, policyEval, "terminal_resize");
		if (invalidSession) return invalidSession;
		...
		try {
			...
			if (daemonGuard) return daemonGuard as ActionResult<{ resized: boolean }>;
			await runtime.resize(options.sessionId, options.cols, options.rows);
			...
			this.invalidateBrokerRuntimeOnError(error);
			const message = error instanceof Error ? error.message : String(error);
			return this.failureWithDebug(`Resize failed: ${message}`, error, {
	...
	async close(
		...
		const invalidSession = await this.rejectInvalidTerminalSessionId<{
			closed: boolean;
		}>(options.sessionId, policyEval, "terminal_close");
		if (invalidSession) return invalidSession;
		...
		try {
			...
			if (
				activeSession &&
				activeSession.terminalSessionId === options.sessionId
			) {
	...
	async resume(options: TermResumeOptions): Promise<ActionResult<unknown>> {
		...
		if (!isPolicyAllowed(policyEval))
			return policyEval as ActionResult<unknown>;
		const invalidSession = await this.rejectInvalidTerminalSessionId<unknown>(
			options.sessionId,
			policyEval,
			"terminal_resume",
		);
