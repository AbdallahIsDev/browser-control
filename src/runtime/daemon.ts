...
import type { ExecOptions, TerminalSessionConfig } from "../terminal/types";
...
export interface DaemonConfig {
	...
	brokerFactory?: (
		daemon: Daemon,
	) => Promise<BrokerServerLike> | BrokerServerLike;
}
...
export class Daemon {
	...
	async start(): Promise<void> {
		...
		this.telemetry.onAlert(
			createTelegramAlertHandler(
				path.join(__dirname, "..", "telegram_notifier.ps1"),
			),
		);
		this.healthCheck =
		...
		if (!criticalOkay) {
			...
			if (criticalFails.length > 0) {
				throw new Error(
					`Daemon start blocked because critical health checks failed: ${JSON.stringify(criticalFails)}`,
				);
			}
		...
		if (!this.chromeConnected) {
			this.log.info(
				"Chrome CDP not available — daemon starting in terminal/FS-only mode (terminal sessions will work)",
			);
		}
		...
		// unresolvable internal aliases) are skipped
		// with a warning. Terminal and FS features work independently
		// of any specific skill.
		try {
			const skillsDir = path.join(__dirname, "..", "skills");
			const loadedSkills =
				await this.skillRegistry.loadFromDirectory(skillsDir);
			if (loadedSkills.length > 0) {
				this.log.info(
					`Loaded ${loadedSkills.length} skill(s) from ${skillsDir}`,
				);
			}
		} catch (error: unknown) {
	...
	async stop(): Promise<void> {
		...
		this.log.info(
			"Shutting down daemon — persisting running tasks and exiting cleanly",
		);
			...
			if (record) {
				...
				if (record.engine) {
					this.memoryStore.set(
						`task_state:${taskId}:current`,
						record.engine.exportState(),
					);
				}
		...
		await Promise.all(
			this.skillBrowserClients.splice(0).map(async (browser) => {
	...
	async submitTask(task: Task, externalId?: string): Promise<string> {
		...
		if (this.policyEngine) {
			...
			const routedStep = this.executionRouter.buildRoutedStep(
				taskIntent,
				action,
				params,
				{
			...
			if (evaluation.decision === "require_confirmation") {
				this.log.warn(
					"Generic task requires confirmation but daemon cannot prompt - denying",
					{
						taskId,
						taskName: task.name,
						action,
	...
	submitSkillTask(
		...
		if (this.policyEngine) {
			...
			if (evaluation.decision === "require_confirmation") {
				...
				this.log.warn(
					"Skill task requires confirmation but daemon cannot prompt - denying",
					{
						taskId,
						skill: skillName,
						action,
	...
	async termExec(
		command: string,
		options: ExecOptions & TerminalSessionConfig & { sessionId?: string } = {},
	): Promise<unknown> {
	...
	async termSnapshot(sessionId?: string): Promise<unknown> {
		...
		const sessions = await Promise.all(
			this.terminalManager.list().map((session) => session.snapshot()),
		);
		return {
	...
	async termResize(
		...
		this.assertOperationAllowed("terminal_resize", { sessionId, cols, rows });
		...
		const pending = store
			...
			.map((sessionId) => {
				...
				const effectiveBuffer =
					this.getTerminalResumePolicy() === "metadata_only" ? null : buffer;
				const decision = this.applyMetadataLosses(
					decideResume(
						sessionId,
						metadata as SerializedTerminalSession | null,
						effectiveBuffer,
					),
					metadata,
				);
	...
	async termResume(sessionId: string): Promise<TerminalResumeResult> {
		...
		if (this.getTerminalResumePolicy() === "abandon") {
			throw new Error(
				"Terminal resume is disabled by TERMINAL_RESUME_POLICY=abandon.",
			);
		}
		const effectiveBuffer =
			this.getTerminalResumePolicy() === "metadata_only" ? null : buffer;
		...
		if (decision.status === "fresh") {
			throw new Error(
				`No persisted state found for terminal session: ${sessionId}`,
			);
		}
		...
		if (!metadata || typeof metadata !== "object") {
			throw new Error(
				`Corrupted persisted state for terminal session: ${sessionId}`,
			);
		}
		...
		const cwdExists = await this.statPath(meta.cwd)
			.then(() => true)
			.catch(() => false);
	...
	async termStatus(
		sessionId: string,
	): Promise<TerminalResumeResult & { session?: Record<string, unknown> }> {
	...
	private assertOperationAllowed(
		...
		const taskIntent: PolicyTaskIntent = {
			...
			actor: "human",
			sessionId:
				typeof params.sessionId === "string" ? params.sessionId : "cli",
			metadata: { source: "cli-daemon" },
		};

		const routed = this.executionRouter.buildRoutedStep(
			taskIntent,
			action,
			params,
			{
	...
	private async recoverInterruptedTasks(): Promise<void> {
		...
		this.log.info(
			`Found ${interruptedIntents.length} interrupted task(s) from previous run`,
		);
		...
		for (const { key, intent } of interruptedIntents) {
			...
			switch (policy) {
				case "resume": {
					...
					const resumed = TaskEngine.resumeFromStore(
						this.memoryStore,
						`task_state:${intent.taskId}:`,
					);
					if (resumed && intent.skill) {
						this.log.info(
							`Task ${intent.taskId} has resumable engine state — re-submitting via skill path`,
						);
						...
						this.log.info(
							`Task ${intent.taskId} has resumable engine state but no skill info — cannot auto-resume. Marking as interrupted.`,
						);
					} else {
						this.log.warn(
							`Cannot resume task ${intent.taskId} — no saved engine state. Marking as interrupted.`,
						);
					}
					break;
				}
				case "reschedule": {
					...
					if (intent.skill) {
						this.log.info(
							`Rescheduling skill task ${intent.taskId} (${intent.skill}/${intent.action})`,
						);
						...
						this.log.warn(
							`Cannot reschedule task ${intent.taskId} — no skill/action info. Marking as interrupted.`,
						);
					}
					break;
				}
	...
	private buildResumeEnv(metadata: SerializedTerminalSession): {
	...
	private async serializeTerminals(): Promise<void> {
		...
		this.log.info(
			`Serializing ${sessions.length} active terminal session(s) before shutdown`,
		);

		for (const session of sessions) {
			try {
				const ptySession = session as PtyTerminalSession & {
					getSerializeableState: () => ReturnType<
						typeof serializeTerminalSession
					>;
				};
				...
				store.saveBuffer(
					...
					captureTerminalBuffer(
						serialized.sessionId,
						state._outputBuffer,
						this.appConfig?.terminalMaxScrollbackLines,
					),
				);
		...
		const evicted = store.enforceMaxSerializedSessions(
			this.appConfig?.terminalMaxSerializedSessions ?? 50,
		);
		if (evicted.length > 0) {
			this.log.info(
				`Evicted ${evicted.length} old serialized terminal session(s)`,
				{
					sessionIds: evicted,
				},
			);
	...
	private async restoreTerminals(): Promise<void> {
		...
		if (resumePolicy === "abandon") {
			this.log.info(
				"Terminal auto-resume is disabled by policy — skipping restoration",
			);
			return;
		}
		...
		this.log.info(
			`Restoring ${pendingIds.length} terminal session(s) from previous run`,
		);
		...
		for (const sessionId of pendingIds) {
			try {
				const { metadata, buffer } = loadPersistedState(store, sessionId);
				const effectiveBuffer =
					resumePolicy === "metadata_only" ? null : buffer;
				const decision = this.applyMetadataLosses(
					decideResume(
						sessionId,
						metadata as SerializedTerminalSession | null,
		...
		try {
			...
			try {
				if (fs.existsSync(debugPath)) {
					const metadata = JSON.parse(fs.readFileSync(debugPath, "utf8")) as {
						port?: number;
					};
					if (metadata.port) {
						port = metadata.port;
			...
			if (isReady) {
				...
				if (this.chromeConnected) {
					...
					if (recovered) {
						...
						this.log.critical(
							"Chrome reconnection failed — marking daemon as degraded",
							{
								maxRetries,
							},
						);
						...
						this.writeDaemonStatus(
							"degraded",
							"Chrome CDP connection lost and reconnection failed",
						);
						...
						this.telemetry.record("chrome.watchdog", "error", 0, {
							...
							message:
								"Chrome CDP connection lost and all reconnection attempts failed",
						});
					}
				}
			}
	...
	checkResourceAlerts(): void {
		const memoryAlertMb =
			this.config.memoryAlertMb ?? this.appConfig?.memoryAlertMb;
		const chromeTabLimit =
			this.config.chromeTabLimit ?? this.appConfig?.chromeTabLimit;

		const memoryUsage = process.memoryUsage();
		const heapUsedMb = Math.round(memoryUsage.heapUsed / (1024 * 1024));

		if (memoryAlertMb && heapUsedMb > memoryAlertMb) {
		...
		if (chromeTabLimit && sessionCount > chromeTabLimit) {
	...
	private async createDefaultBroker(): Promise<BrokerServerLike> {
		...
		if (
			!("createBrokerServer" in module) ||
			typeof module.createBrokerServer !== "function"
		) {
		...
		const broker = module.createBrokerServer({
			callbacks: {
				...
				pauseScheduledTask: async (id) => {
					...
					return (
						this.scheduler.getQueue().find((entry) => entry.id === id) ?? null
					);
				},
				...
				handleTerminal: async (subcommand, payload) => {
					switch (subcommand) {
						...
						case "exec":
							return this.termExec(
								payload.command as string,
								payload as ExecOptions &
									TerminalSessionConfig & { sessionId?: string },
							);
						case "type":
		...
		// Wire up terminal output streaming to the broker broadcast.
		// NOTE: Term output is broadcast to ALL authenticated broker WebSocket
		// clients, not per-session subscribers. This is intentional: broker
		// clients share a single API key and are trusted operators. Term output
		// is shared observability data. Raw output is not logged.
		this.terminalManager.onData((sessionId, data) => {
			broker.broadcast({ type: "terminal.output", sessionId, data });
		});
	...
	private async buildSkillContext(
		...
		const connection = sessionId
			? this.stagehandManager.getSession(sessionId)
			: undefined;
		...
		if (skill.manifest.requiresFreshPage && connection) {
			...
			if (!reconnected) {
				...
				if (port) {
					try {
						...
						contextErrors.push(
							`manager attach failed: ${error instanceof Error ? error.message : String(error)}`,
						);
			...
			if (!browserContext) {
				...
				if (port) {
					try {
						...
						contextErrors.push(
							`manager context attach failed: ${error instanceof Error ? error.message : String(error)}`,
						);
			...
			if (!browserContext) {
				...
				if (port) {
					try {
						const directBrowser = await chromium.connectOverCDP(
							`http://127.0.0.1:${port}`,
						);
						this.skillBrowserClients.push(directBrowser);
						browserContext =
							directBrowser.contexts()[0] ?? (await directBrowser.newContext());
					} catch (error: unknown) {
						contextErrors.push(
							`direct CDP failed: ${error instanceof Error ? error.message : String(error)}`,
						);
	...
	private async executeSkillAsync(
		...
		try {
			...
			if (!builtResult) {
				throw new Error(
					`Cannot build context for skill "${skillName}" — no page available.`,
				);
			}
			...
			if (skill?.restoreState) {
				const savedState = this.memoryStore.get<Record<string, unknown>>(
					`skill:${skillName}:state`,
				);
				if (savedState) {
			...
			if (skill?.onError && skillContext) {
				try {
					...
					this.log.warn("onError hook failed", {
						skillName,
			...
			if (skillContext) {
				await this.skillRegistry
					.teardown(skillName, skillContext)
					.catch(() => {});
