...
const terminalSessionIdProperty = {
  type: "string",
  description: "Terminal session ID from bc_terminal_open.",
} as const;

function useBrowserControlSession(api: BrowserControlAPI, params: Record<string, unknown>): void {
...
export function buildTerminalTools(api: BrowserControlAPI): McpTool[] {
  return [
...

    {
      ...
      description: "Execute a command in a terminal session, or run a one-shot command. For one-shot execution, omit terminalSessionId. For persistent session execution, provide the terminal session ID from bc_terminal_open.",
      inputSchema: buildSchema({
      ...
      handler: async (params) => {
        ...
        return api.terminal.exec({
          ...
          sessionId: params.terminalSessionId as string | undefined,
...

    {
      ...
      handler: async (params) => {
        ...
        return api.terminal.read({
          sessionId: params.terminalSessionId as string,
