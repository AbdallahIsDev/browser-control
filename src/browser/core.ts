...
function isWslReachableHostCandidate(hostOrUrl: string): boolean {
	...
	return (
		host === DEFAULT_LOCALHOST || host.startsWith("127.") || isPrivateIpv4(host)
	);
}
...
function readDebugInteropState(): DebugInteropState | null {
	try {
		...
		return JSON.parse(
			fs.readFileSync(debugInteropPath, "utf8"),
		) as DebugInteropState;
	} catch {
		return null;
	}
}
...
export function getDebugEndpointCandidates(
	...
	const rawMetadata =
		options.metadata === undefined ? readDebugInteropState() : options.metadata;
	...
	if (isWsl) {
		...
		pushUniqueCandidate(
			candidates,
			metadata?.localhostUrl ?? DEFAULT_LOCALHOST,
			port,
		);
		pushUniqueCandidate(
			candidates,
			metadata?.windowsLoopbackUrl ?? DEFAULT_DEBUG_HOST,
			port,
		);
		return candidates;
	}
...
export async function createAutomationContext(
	...
	if (stealthRequested) {
		context = await createStealthContext(browser, {
			...
			fingerprintSeed: getFirstDefinedString(
...
export async function ensureContextHasPage(
	context: BrowserContext,
): Promise<Page> {
...
export function findPageByUrl(
	...
	return (
		getAllPages(browser, context).find((page: Page) =>
			page.url().includes(urlPattern),
		) ?? null
	);
}
...
export function getFramerPage(
	...
	if (!page) {
		throw new Error(
			"No Framer tab was found. Open the Framer editor in the persistent Chrome session first.",
		);
	}
	return page;
}
...
async function maybeRestoreSessionCookies(
	...
	try {
		...
		log.error(
			`Failed to restore session cookies for "${options.sessionKey}": ${error instanceof Error ? error.message : String(error)}`,
		);
	}
}
...
function attachSessionCookiePersistence(
	...
	if (
		!options.memoryStore ||
		!options.sessionKey ||
		options.persistSessionCookies === false
	) {
		return;
	}
	...
	const saveCookies = async (): Promise<void> => {
		try {
			...
			log.error(
				`Failed to save session cookies for "${options.sessionKey}": ${error instanceof Error ? error.message : String(error)}`,
			);
		}
	};
...
export async function rawDomClick(
	page: Page,
	selector: string,
): Promise<boolean> {
	try {
...
export async function keyboardFill(
	page: Page,
	selector: string,
	value: string,
): Promise<boolean> {
...
export async function isDebugPortReady(port = 9222): Promise<boolean> {
	try {
		...
		log.error(
			`Debug port check failed: ${error instanceof Error ? error.message : String(error)}`,
		);
		return false;
	}
}
