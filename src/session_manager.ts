 ...
 */
export function isPolicyAllowed(result: PolicyEvalResult): result is PolicyAllowResult {
...
export class LocalTerminalRuntime implements TerminalRuntime {
  ...
  async open(config: { shell?: string; cwd?: string; name?: string }) {
    ...
    return { id: session.id, shell: session.shell, cwd: session.cwd, status: session.status };
  }

  async exec(command: string, options: { sessionId?: string; timeoutMs?: number }) {
    if (options.sessionId) {
      const session = this.tm.get(options.sessionId);
      if (!session) throw new Error(`Terminal session not found: ${options.sessionId}`);
...
export class BrokerTerminalRuntime implements TerminalRuntime {
  ...
  get brokerUrl(): string { return this._brokerUrl; }
  ...
  private async fetchJson(url: string, options: RequestInit): Promise<unknown> {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        const isTransient = /ECONNREFUSED|fetch failed|ECONNRESET|EPIPE|socket hang up/i.test(message);
  ...
  async open(config: { shell?: string; cwd?: string; name?: string }) {
    const raw = await this.request("open", {
      shell: config.shell,
      cwd: config.cwd,
      name: config.name,
  ...
  async exec(command: string, options: { sessionId?: string; timeoutMs?: number }) {
    const raw = await this.request("exec", {
      command,
      sessionId: options.sessionId,
      timeoutMs: options.timeoutMs,
    }, "POST") as Record<string, unknown>;
  ...
  async read(sessionId: string, maxBytes?: number) {
    const result = await this.request("read", { sessionId, maxBytes }) as Record<string, unknown>;
  ...
  async list() {
    const raw = await this.request("sessions") as Array<Record<string, unknown>>;
    return raw.map((s) => ({
      id: (s.id as string) ?? "",
      name: (s.name as string) ?? undefined,
      shell: (s.shell as string) ?? "",
...
export class SessionManager {
  ...
  async create(name: string, options: {
    ...
    try {
      ...
      try { this.policyEngine.setProfile(previousProfile); } catch { /* best-effort */ }
  ...
  list(): ActionResult<SessionListEntry[]> {
    const entries: SessionListEntry[] = Array.from(this.sessions.values()).map((s) => ({
      id: s.id,
      name: s.name,
  ...
  use(nameOrId: string): ActionResult<SessionState> {
    ...
    if (!state) {
      state = Array.from(this.sessions.values()).find((s) => s.name === nameOrId);
    }
   ...
   */
  async ensureDaemonRuntime(options: {
    ...
    if (running) {
      ...
      for (let i = 1; i <= maxTerminalRetries; i++) {
        ...
        if (termReady) break;
        await new Promise((resolve) => setTimeout(resolve, terminalRetryDelayMs));
      }
      ...
      log.warn("Daemon health OK but terminal endpoint not ready after retries");
      return false;
    }
    ...
    if (options.autoStart) {
      try {
        ...
        daemonProcess.stderr?.on("data", (chunk: Buffer) => errorChunks.push(chunk));
        ...
        if (exited) {
          ...
          log.warn("Daemon auto-start failed", { error: errorOutput || "Process exited immediately" });
        ...
        log.warn("Daemon auto-started but did not become terminal-ready in time");
        return false;
      } catch (error: unknown) {
        log.warn("Daemon auto-start failed", { error: error instanceof Error ? error.message : String(error) });
        return false;
      }
    }
  ...
  private resolveSession(nameOrId?: string): SessionState | null {
    ...
    if (byId) return byId;
    // Try name
    return Array.from(this.sessions.values()).find((s) => s.name === nameOrId) ?? null;
  }
  ...
  private persistSession(state: SessionState): void {
    try {
      ...
      log.warn(`Failed to persist session state: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  private loadPersistedSessions(): void {
    try {
      ...
      log.warn(`Failed to load persisted sessions: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
}
