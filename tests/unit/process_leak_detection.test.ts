...
async function createTestEnv(): Promise<{
  ...
  const env: Record<string, string> = {
    ...
    BROKER_API_KEY: "process-leak-test-key",
    LOG_FILE: "false",
    ...
    try {
      ...
      if (resp.ok) {
        ...
        const termResp = await fetch(`${brokerUrl}/api/v1/term/sessions`, {
          ...
          signal: AbortSignal.timeout(2000),
          headers: { Authorization: `Bearer ${env.BROKER_API_KEY}` },
        });
...
function countBcAutomationChrome(home?: string): number {
  ...
  try {
    ...
    const homeNeedle = home?.toLowerCase();
    for (const line of output.split("\n")) {
      ...
      if (
        ...
        (homeNeedle ? lowerLine.includes(homeNeedle) : lowerLine.includes(".browser-control"))
...
describe("Process leak detection (Section 5 / Section 12)", () => {
  ...
  it("bc term open → close does not leak daemon or shell processes", async () => {
    ...
    const chromeBefore = countBcAutomationChrome(env.BROWSER_CONTROL_HOME);
    ...
    const chromeAfter = countBcAutomationChrome(env.BROWSER_CONTROL_HOME);

    assert.ok(
      chromeAfter <= chromeBefore,
      `BC automation Chrome count should not increase. Before: ${chromeBefore}, After: ${chromeAfter}`,
    );
  ...
  it("bc term open with force-killed daemon does not leak processes", async () => {
    ...
    assert.ok(
      chromeAfter <= chromeBefore,
      `BC automation Chrome count should not increase after force-kill. Before: ${chromeBefore}, After: ${chromeAfter}`,
    );
