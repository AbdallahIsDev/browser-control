...
test("snapshot boxes survive null Playwright viewport metadata", async () => {
	...
	try {
		...
		await page.setContent(
			`<button id="confirm" style="width: 120px; height: 40px">Confirm</button>`,
		);
		...
		const snap = await snapshot(page, {
			sessionId: "browser-features",
			boxes: true,
		});
		const button = snap.elements.find(
			(element) => element.role === "button" && element.name === "Confirm",
		);
...
test("screencast state and timeline restore across recorder instances", async () => {
	const home = fs.mkdtempSync(
		path.join(os.tmpdir(), "bc-browser-features-screencast-"),
	);
	...
	try {
		...
		const timelinePath = stopped.timelinePath;
		assert.ok(
			timelinePath,
			"timeline should be written from restored memory store events",
		);
...
test("source guards cover prior browser feature review regressions", () => {
	...
	assert.match(
		screencast,
		/this\.store\.keys\(OBSERVABILITY_KEYS\.screencastPrefix\)/,
	);
	...
	assert.match(
		actions,
		/data:\s*droppedData\.length > 0\s*\?\s*droppedData\.map/,
	);
