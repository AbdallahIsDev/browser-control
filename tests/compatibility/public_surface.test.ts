...
test("CLI inventory includes required public commands", async () => {
	...
	const syntaxes = inventory.sections.flatMap((section) =>
	...
	for (const required of [
		...
		assert.ok(
			syntaxes.some(
				(syntax) => syntax === required || syntax.startsWith(`${required} `),
			),
...
test("MCP tool inventory has unique names and serializable schemas", () => {
	...
	for (const tool of inventory.tools) {
		assert.ok(
			...
			`MCP tool ${tool.name} must have description.`,
		);
		assert.notEqual(
			tool.category,
			"unknown",
			`MCP tool ${tool.name} must have category.`,
		);
		assert.doesNotThrow(
			() => JSON.stringify(tool.inputSchema),
			`MCP tool ${tool.name} schema must be JSON serializable.`,
		);
	}
});
...
test("config inventory and .env.example stay aligned", () => {
	...
	const configEnvVars = new Set(
		inventory.configKeys.flatMap((entry) => entry.envVars),
	);
...
test("ActionResult core fields remain present in success and failure results", () => {
	const success = formatActionResult(
		successResult({ ok: true }, { path: "command", sessionId: "s1" }),
	);
	const failure = formatActionResult(
		failureResult("bad", { path: "a11y", sessionId: "s1" }),
	);
...
test("persisted format inventory uses schema placeholders, not machine values", () => {
	...
	assert.ok(
		!normalizedText.includes(os.homedir()),
		"persisted format inventory must not snapshot user home.",
	);
	assert.ok(
		!normalizedText.includes(process.cwd()),
		"persisted format inventory must not snapshot repo path.",
	);
