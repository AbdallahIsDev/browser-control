...
function getBrowserControlTypeSignatures(existingProgram?: ts.Program, existingChecker?: ts.TypeChecker): unknown {
  const program = existingProgram ?? ts.createProgram({
    rootNames: [path.join(process.cwd(), "browser_control.ts"), path.join(process.cwd(), "src", "browser_control.ts")],
    options: {
      target: ts.ScriptTarget.ES2022,
      module: ts.ModuleKind.CommonJS,
      moduleResolution: ts.ModuleResolutionKind.Node10,
  ...
  const sourceFile = program.getSourceFile(path.join(process.cwd(), "src", "browser_control.ts"))
    ?? program.getSourceFile(path.join(process.cwd(), "browser_control.ts"));
...
export function getActionResultContract(): unknown {
  ...
  const failure = failureResult("sample error", {
    ...
    errorMetadata: { tabId: "missing-tab" },
