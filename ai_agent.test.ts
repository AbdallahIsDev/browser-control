...
test("requireConfirmation skips destructive click actions", async () => {
  const actions: string[] = [];
  ...
  const result = await agent.executeGoal("Test confirmation", page, 3);

  assert.equal(result.success, true);
  assert.deepEqual(actions, []); // No click on submit-order was executed
  assert.equal(result.decisions[0].action, "click"); // Decision was still recorded
});

test("requireConfirmation skips destructive fill actions", async () => {
  const actions: string[] = [];
  ...
  const result = await agent.executeGoal("Test confirmation fill", page, 3);

  assert.equal(result.success, true);
  assert.deepEqual(actions, []); // No fill on credit-card was executed
});
