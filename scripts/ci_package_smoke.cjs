...
for (const filePath of walkFiles(distDir).filter((file) =>
	/\.(?:js|d\.ts)$/.test(file),
)) {
	const content = fs.readFileSync(filePath, "utf8");
	if (content.includes("@bc/")) {
		fail(
			`Built file contains unresolved @bc path alias: ${path.relative(root, filePath)}`,
		);
	}
}
...
try {
	...
	const files = new Set(
		(packInfo.files ?? []).map((file) => file.path.replace(/\\/g, "/")),
	);
	...
	if (errors.length === 0) {
		...
		runNpm(["install", "--omit=dev", "--no-audit", "--no-fund", tarballPath], {
			cwd: tempProject,
		});
		...
		const binPath = path.join(
			...
			"node_modules",
			".bin",
			process.platform === "win32" ? "bc.cmd" : "bc",
		);
...
console.log(
	`Package smoke passed: ${packInfo?.files?.length ?? 0} files checked.`,
);
