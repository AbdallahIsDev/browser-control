...
7. Before creating automation helper scripts, read `C:\Users\11\.browser-control\automation-helpers\registry.json` if it exists and reuse matching helpers by site/domain/task/failure type.
8. If no helper fits and a new helper is created, register it in `C:\Users\11\.browser-control\automation-helpers\registry.json` with site, task tags, file paths, usage, and purpose.
