...
const state = {
	page: localStorage.getItem("bc-page") || "command",
      ...
      <aside class="rail">
        <div class="brand">
          ...
          <div>
            ...
            <div class="brand-sub">Local AI work OS</div>
          </div>
        </div>
        <nav class="nav-list">
          ${pages
      ...
      <main class="workspace">
        <header class="topbar">
          <div>
            ...
            <h1>${esc(pages.find(([key]) => key === state.page)?.[1] || "Command")}</h1>
          </div>
        ...
        ${
					state.tasks.length
						? table(state.tasks, [
								...
								["Status", (row) => row.status || "queued"],
								[
									"Updated",
									(row) => row.updatedAt || row.completedAt || row.startedAt,
								],
							])
						: emptyState("No task history yet.")
				}
      </section>
      <section class="panel">
        <div class="panel-title">Live Progress</div>
        ${state.events.length ? state.events.slice(-12).reverse().map(activityRow).join("") : emptyState("No live events yet.")}
      </section>
    </div>`;
}
...
function renderBrowser() {
    ...
    <div class="page-stack">
      <section class="panel compact-form">
        ...
        <div class="command-actions">
          <button class="primary" id="browser-open">Open</button>
          <button id="browser-snapshot">Snapshot</button>
          <button id="browser-screenshot">Screenshot</button>
        </div>
        <div id="browser-output" class="doctor-output">No browser action run yet.</div>
      </section>
    </div>`;
	document
		.getElementById("browser-open")
		.addEventListener("click", async () => {
			const url = document.getElementById("browser-url").value.trim();
			if (!url) return setNotice("Enter a URL.");
	...
	document
		.getElementById("browser-snapshot")
		.addEventListener("click", async () => {
			await runBrowserAction("/api/browser/snapshot", {}, "Snapshot captured.");
		});
	document
		.getElementById("browser-screenshot")
		.addEventListener("click", async () => {
			await runBrowserAction(
...
function renderWorkflows() {
    ...
    <div class="split">
      <section class="panel">
        <div class="panel-title">Workflow Runs</div>
        ${
					state.events.filter((event) => /workflow|task/.test(event.type || ""))
						.length
						? state.events
								.filter((event) => /workflow|task/.test(event.type || ""))
								.slice(-12)
								.reverse()
								.map(activityRow)
								.join("")
						: emptyState("No workflow events yet.")
				}
      </section>
      <section class="panel">
        <div class="panel-title">Reusable Memory</div>
        ${automationCards(state.automations.slice(0, 5))}
      </section>
    </div>`;
}
...
function renderPackages() {
	document.getElementById("view").innerHTML = `
    <section class="panel">
      <div class="panel-title">Automation Packages</div>
      ${
				state.packages.length
					? table(state.packages, [
							["Package", (row) => row.name],
							["Version", (row) => row.version],
							["Installed", (row) => row.installedAt],
						])
					: emptyState("No installed packages reported.")
			}
    </section>`;
}
...
function renderEvidence() {
    ...
    <div class="split">
      <section class="panel">
        <div class="panel-title">Debug Bundles</div>
        ${
					state.debugBundles.length
						? table(state.debugBundles, [
								["Bundle", (row) => row.bundleId],
								["Task", (row) => row.taskId],
								["Assembled", (row) => row.assembledAt],
							])
						: emptyState("No debug bundles found.")
				}
      </section>
      <section class="panel">
        ...
        ${
					state.logs.length
						? state.logs
								...
								.map(
									(line) =>
										`<div class="activity-row"><strong>${esc(line)}</strong></div>`,
								)
								.join("")
...
function renderSettings() {
    ...
    <section class="panel settings-panel spaced">
      <div class="panel-title">Storage</div>
      <div class="doctor-output">${esc(state.dataHomeReport?.home || runtimeSummary().dataHome || "Unknown data home")}</div>
      <button id="storage-refresh">Refresh storage report</button>
    </section>`;
	document
	...
	document
		...
		.addEventListener("click", refreshStorageReport);
}
...
function renderAdvanced() {
    ...
    <div class="split">
      <section class="panel">
        <div class="panel-title">Runtime Internals</div>
        ${table(
					[
						{ name: "Policy", value: state.status?.policyProfile },
						{ name: "Data home", value: state.status?.dataHome },
						{
							name: "Broker",
							value: state.status?.broker?.reachable ? "reachable" : "offline",
						},
						{ name: "Daemon", value: state.status?.daemon?.state },
					],
					[
						["Name", (row) => row.name],
						["Value", (row) => row.value],
      ...
      <section class="panel">
        ...
        ${state.events.length ? state.events.slice(-10).reverse().map(activityRow).join("") : emptyState("No events yet.")}
      </section>
    </div>`;
}
...
async function runBrowserAction(path, body, successMessage) {
	setNotice("Running browser action...");
	try {
		...
		const output =
			data.title || data.url || data.path || result.error || successMessage;
		document.getElementById("browser-output").textContent = output;
		setNotice(successMessage);
		await refreshAll();
	} catch (error) {
		document.getElementById("browser-output").textContent = error.message;
...
async function refreshStorageReport() {
	state.dataHomeReport = await api("/api/data/doctor").catch(() => null);
	setNotice("Storage report refreshed.");
	render();
}

function render() {
	renderShell();
	if (state.page === "command") renderAgent();
...
async function refreshAll() {
	try {
		const [
			...
			api("/api/debug/bundles").catch(() => []),
			api("/api/logs?maxLines=40").catch(() => []),
			api("/api/data/doctor").catch(() => null),
		]);
		state.status = status;
		...
		state.debugBundles = Array.isArray(debugBundles) ? debugBundles : [];
		state.logs = Array.isArray(logs) ? logs : [];
		state.dataHomeReport = dataHomeReport;
